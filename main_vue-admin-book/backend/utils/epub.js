var xml2js = require('xml2js');  // 解压库
var xml2jsOptions = xml2js.defaults['0.1'];
var EventEmitter = require('events').EventEmitter;   // node自带事件库
...
class EPub extends EventEmitter {
  ...
  open() {
    try {
      ...
      // 如果发现文件不存在的时候，就会emit一个error事件
      // emit:表示分发一个error事件，但是不关注在哪里进行消费
  ...
  **/
  ...
  getFile(id, callback) {
    // 判断manifest的id 
    if (this.manifest[id]) {
      // 现在的epub文件仍然是个压缩包 ，所以需要zip下面提供的api readFile 把href取出来（资源路径）
      // 还传了个回调方法
      this.zip.readFile(this.manifest[id].href, (function (err, data) {
        if (err) {
          callback(new Error("Reading archive failed"));
        ...
        // 没出错的话给callback三个参数  第一个参数为null 因为他是传入的error
